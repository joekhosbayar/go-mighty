...
func (s *Store) CreateUser(ctx context.Context, user *User) error {
	...
	_, err := s.db.ExecContext(ctx, query, user.ID, user.Username, user.PasswordHash, user.Email, user.CreatedAt, user.UpdatedAt)
	if err != nil {
		return err
	}
	...
	_, err = s.db.ExecContext(ctx, queryStats, user.ID)
	return err
}
